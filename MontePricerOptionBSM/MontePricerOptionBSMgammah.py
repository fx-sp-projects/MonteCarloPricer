...
for i in range(Nrsims):
    ...
    for j in range(Nrsteps):
        gammaS_t = gamma('c', np.exp(lnS_t), K, T - j * dt, r, vol)
        lnS_tn = lnS_t + nuxdt + volxsqdt * np.random.normal()  # GBM
        # z = halton_norm(1)
        # lnS_t = lnS_t + nuxdt + volxsqdt * float(z[0])  # GBM
        c_v2 = c_v2 + gammaS_t*((np.exp(lnS_tn) - np.exp(lnS_t))**2-ergamma*np.exp(lnS_t)**2)
        lnS_t = lnS_tn
    # Calc Payoff and average
    S_T = np.exp(lnS_t)   # GBM
    C_T = max(0, S_T - K) + beta2*c_v2
    sum_C_T = sum_C_T + C_T
